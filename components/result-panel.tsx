...
export function ResultPanel({
  ...
  if (!result) {
    return (
      <section className="rounded-[2rem] bg-white p-8 shadow-soft ring-1 ring-slate-200/70 md:p-10">
        ...
        <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">Paste a listing, extract details, then get the import cost and deal verdict.</p>
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-[2rem] bg-ink p-7 text-white shadow-soft md:p-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Total import cost</p>
      ...
      <Disclosure title="Assumptions and feedback">
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="space-y-2 text-sm text-slate-600">
            {(result.warnings.length > 0 ? result.warnings : result.risk.reasons).map((warning) => (
              <p key={warning} className="rounded-xl bg-amber-50 px-4 py-3 text-amber-900">{warning}</p>
            ))}
            <div className="rounded-xl bg-ink p-4 text-slate-200">
