...
function encodePayload(payload: ImportScoreExtension.ExtractionPayload): string {
  return btoa(unescape(encodeURIComponent(JSON.stringify(payload))));
}

function buildAppUrl(payload: ImportScoreExtension.ExtractionPayload): string {
  const url = new URL(APP_URL);
  url.hash = `extensionPayload=${encodeURIComponent(encodePayload(payload))}`;
  return url.toString();
}

function badgeForStatus(status: ImportScoreExtension.ExtractionStatus): { text: string; color: string } {
...
async function extractFromTab(tabId: number): Promise<ImportScoreExtension.ExtractionPayload> {
  ...
  const results = await chrome.scripting.executeScript({
    target: { tabId },
    func: () => (globalThis as any).ImportScoreExtension.extractCurrentPage()
  });

  return results?.[0]?.result as ImportScoreExtension.ExtractionPayload;
}
