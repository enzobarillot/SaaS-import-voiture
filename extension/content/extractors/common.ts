...
(() => {
  ...
  function applyField(fields: Record<string, unknown>, field: string, value: unknown): void {
  ...
  function collectPath(source: unknown, path: string[]): unknown[] {
    if (!source) return [];
    if (path.length === 0) return [source];
    if (Array.isArray(source)) return source.flatMap((entry) => collectPath(entry, path));
    if (typeof source !== "object") return [];
    const [head, ...tail] = path;
    return collectPath(objectValue(source as Record<string, unknown>, head), tail);
  }
  ...
  function stringify(value: unknown): string | undefined {
    ...
    if (value && typeof value === "object") {
      for (const key of ["name", "label", "displayName", "value", "text", "title"]) {
        const found = stringify(objectValue(value as Record<string, unknown>, key));
  ...
  function firstString(records: Array<Record<string, unknown>>, paths: string[][]): string | undefined {
  ...
  function firstNumber(records: Array<Record<string, unknown>>, paths: string[][]): number | undefined {
    for (const record of records) {
      for (const path of paths) {
        for (const value of collectPath(record, path)) {
  ...
  function normalizeFuel(text?: string): string | undefined {
  ...
  function normalizeTransmission(text?: string): string | undefined {
    if (!text) return undefined;
    const value = normalize(text);
  ...
  function normalizeSeller(text?: string): string | undefined {
    if (!text) return undefined;
    const value = normalize(text);
  ...
  function normalizeVat(text?: string): string | undefined {
    if (!text) return undefined;
    const value = normalize(text);
  ...
  function horsepowerFromText(text?: string): number | undefined {
    if (!text) return undefined;
  ...
  function co2FromText(text?: string): number | undefined {
    return parseNumber(text?.match(/(?:co2|emissions?)[^0-9]{0,30}([0-9]{2,3})/i)?.[1]);
  }

  function formatModel(tokens: string[]): string {
    return tokens.map((token) => (/^[a-z]+$/i.test(token) ? token.charAt(0).toUpperCase() + token.slice(1) : token)).join(" ");
  }

  function findSequence(tokens: string[], sequence: string[], startAt = 0): number {
  ...
  function trimFromTokens(tokens: string[], startAt = 0): { trim?: string; index: number } {
  ...
  function keywordTokens(text: string): string[] {
    return normalize(safeDecode(text))
      .replace(/[^a-z0-9]+/g, " ")
      .split(" ")
      .filter(Boolean)
      .map((token) => token.replace(/^0+(?=\d)/, ""))
      .filter((token) => (token.length > 1 || SINGLE_LETTERS.has(token)) && !/^\d{5,}$/.test(token) && !NOISE_WORDS.has(token));
  }

  function inferFromKeywords(text: string): Record<string, unknown> {
  ...
  function usefulUrlText(url: URL): string {
    const segments = url.pathname
      .split("/")
      .map((segment) => safeDecode(segment).trim())
      .filter((segment) => segment.length > 0 && !/^\d{5,}$/.test(segment));
    const params = Array.from(url.searchParams.entries())
      .filter(([key, value]) => value.length <= 120 && !/^(?:id|ad|adid|listing|listingid|offer|offerid|ref|utm_.+)$/i.test(key))
      .filter(([key, value]) => /brand|make|model|trim|year|fuel|transmission|mileage|km|power|hp|ch|ps|kw/i.test(key) || /[a-z]/i.test(value))
      .map(([key, value]) => `${key} ${value}`);
    return [...segments, ...params].join(" ").replace(/\.[a-z0-9]{2,5}\b/gi, " ").replace(/[_+/=]/g, " ");
  }

  function jsonLdRecords(): Array<Record<string, unknown>> {
    ...
    document.querySelectorAll('script[type="application/ld+json"]').forEach((script) => {
      try {
        ...
        // Ignore malformed vendor blocks.
      }
    });
    return records;
  }
  ...
  function normalizeRecords(value: unknown): Array<Record<string, unknown>> {
    ...
    return Array.isArray(record["@graph"]) ? [record, ...normalizeRecords(record["@graph"])] : [record];
  }

  function metaTags(): Record<string, string> {
  ...
  function factsFromDom(config: ImportScoreExtension.ExtractorConfig): Array<{ label: string; value: string }> {
  ...
  function applyText(fields: Record<string, unknown>, text: string, includeIdentity: boolean): void {
    if (!text.trim()) return;
    if (includeIdentity) {
  ...
  function applyFacts(fields: Record<string, unknown>, facts: Array<{ label: string; value: string }>): void {
  ...
  function extractStructured(fields: Record<string, unknown>, records: Array<Record<string, unknown>>): void {
  ...
  function selectedText(selectors: string[] = []): string {
    return selectors
  ...
  function extractGeneric(config: ImportScoreExtension.ExtractorConfig): ImportScoreExtension.ExtractionPayload {
    const confirmed: Record<string, unknown> = {};
    const inferred: Record<string, unknown> = { countryOfOrigin: config.countryOfOrigin, listingUrl: location.href };
    ...
    let status: ImportScoreExtension.ExtractionStatus = "failed";
    ...
    } as ImportScoreExtension.ExtractionPayload;
  }

  function unsupportedPayload(): ImportScoreExtension.ExtractionPayload {
    return {
      source: "unknown",
